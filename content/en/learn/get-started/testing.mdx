...
WebNN testing encompasses multiple validation approaches to cover various [test suites](#webnn-test-suites) to ensure correctness, performance, and stability.

### Test Infrastructure

- **Chromium bots**: Continuous integration testing (in progress)
- Microsoft Edge testing facilities
- Hardware vendor testing facilities

### Backend Versions

WebNN is validated against multiple versions of backends implementation:

- **Public releases**: Stable Windows ML, Execution Provider, and GPU, NPU driver
- **Pre-release versions**: Early access builds
...
- **Pre-release**: PageHeap testing before each major release
