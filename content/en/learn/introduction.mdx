...
> <InfoIcon /> WebNN utilizes graph-based computation that is optimally suited for NPU acceleration, while also supporting GPU and CPU.

> <InfoIcon /> WebNN is currently the only web API that enables NPU access.
...
WebNN automatically utilizes available hardware acceleration (CPU, GPU, or dedicated NPU processors) for neural network operations. This optimization occurs transparently across different devices and operating systems, maximizing inference speed without requiring hardware-specific code.
 ...
 - Multi-hardware acceleration support - WebNN utilizes graph-based computation that is optimally suited for NPU acceleration, while also supporting GPU and CPU.
 - Unique NPU access - Currently WebNN is the only web API providing access to NPU hardware acceleration
