...
## Run WebNN on Windows ML backend
...
## Run WebNN on Windows ML backend (NPU)

<Steps>
### Launch Chrome or Edge browser with switch [`--disable_webnn_for_npu=0`](../../api-reference/browser-compatibility/chrome-flags#--disable_webnn_for_npu)
</Steps>
...
> <InfoIcon /> The Windows App SDK and Execution Providers will be installed automatically in the background when the `ONNX Runtime backend for WebNN` is enabled.
