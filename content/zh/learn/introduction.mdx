import 架构DiagramIntro from '../../../app/_components/architecture/architecture-intro.jsx'
import InfoIcon from '../../../app/_components/icons/info.jsx'

# WebNN API 简介

了解 WebNN API、其工作原理以及如何使用它。

WebNN API 是一项新的 Web 标准，允许 Web 应用程序和框架利用设备上的硬件（如 GPU、CPU 或专用的 AI 加速器）来加速深度神经网络。

> <InfoIcon /> WebNN 利用了非常适合 NPU 加速的基于图的计算，同时也支持 GPU 和 CPU。

> <InfoIcon /> WebNN 目前是唯一能够访问 NPU 的 Web API。
...
WebNN automatically utilizes available hardware acceleration (CPU, GPU, or dedicated NPU processors) for neural network operations. This optimization occurs transparently across different devices and operating systems, maximizing inference speed without requiring hardware-specific code.
 ...
 - Multi-hardware acceleration support - WebNN 利用了非常适合 NPU 加速的基于图的计算，同时也支持 GPU 和 CPU。
